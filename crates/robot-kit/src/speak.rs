...
impl SpeakTool {
    pub fn new(config: RobotConfig) -> Self {
        let audio_dir = directories::UserDirs::new()
            .map(|d| d.home_dir().join(".velaclaw/tts_cache"))
            .unwrap_or_else(|| PathBuf::from("/tmp/velaclaw_tts"));
    ...
    async fn speak(&self, text: &str, emotion: &str) -> Result<()> {
        ...
        let model_path = directories::UserDirs::new()
            .map(|d| {
                d.home_dir()
                    .join(format!(".velaclaw/models/piper/{}.onnx", voice))
            })
    ...
    async fn play_sound(&self, sound: &str) -> Result<()> {
        let sounds_dir = directories::UserDirs::new()
            .map(|d| d.home_dir().join(".velaclaw/sounds"))
            .unwrap_or_else(|| PathBuf::from("/usr/local/share/velaclaw/sounds"));
