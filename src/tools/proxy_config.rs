    ...
    async fn handle_set(&self, args: &Value) -> anyhow::Result<ToolResult> {
        ...
        if let Some(scope_raw) = args.get("scope") {
            ...
            proxy.scope = Self::parse_scope(scope).ok_or_else(|| {
                anyhow::anyhow!("Invalid scope '{scope}'. Use environment|velaclaw|services")
            })?;
        }
...
impl Tool for ProxyConfigTool {
    ...
    fn description(&self) -> &str {
        "Manage VelaClaw proxy settings (scope: environment | velaclaw | services), including runtime and process env application"
    }
    ...
    fn parameters_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "scope": {
                    "type": "string",
                    "description": "Proxy scope: environment | velaclaw | services"
                },
                "http_proxy": {
                    "type": ["string", "null"],
...
mod tests {
    ...
    async fn list_services_action_returns_known_keys() {
        ...
        let result = tool
            .execute(json!({"action": "list_services"}), &ToolExecutionContext::default())
            .await
            .unwrap();
    ...
    async fn set_and_get_round_trip_proxy_scope() {
        ...
        let get_result = tool.execute(json!({"action": "get"}), &ToolExecutionContext::default()).await.unwrap();
