...
DAG-related keys below: **`template_dag` / candidate emit / shadow** stay library/doctor (AI-DAG frozen off the default turn). **`bounded_dag_live` is retained and ignored for scheduling** (VL-RAO-001). CLI and Web turns use `run_tool_call_loop`. The key does not start a planner or linear work nodes, and the process does not rewrite it.
...
| `max_tool_iterations` | `10` | Maximum tool-call rounds per user message. Reaching the limit returns the visible assistant text plus a notice that the reply is incomplete. It does not discard that text. `0` still means this default of 10 |
| `max_history_messages` | `50` | Maximum conversation history messages retained per session |
| `compact_context_ratio` | `0` | **VL-RAO-002.** Optional second compact trigger inside `prepare_turn_history`. `0` keeps the message-count rule only. A positive fraction (for example `0.75`) also summarizes when a rough estimate (about four characters per token) reaches that fraction of the protocol `context_window`. A missing window does not fire this trigger and does not invent a 24k window. The same summarizer, keep-recent splice, and 2000-character summary cap stay in place. This does not change `assemble_layered` or the `compact_context` 8192 emergency budget. Channel turns pass the routed model's window into the same entry. **VL-RAO-005:** after tool results are appended, the loop calls the same function with layered assembly off, using this ratio and `max_history_messages`. |
| `macro_stages` | `[]` | **VL-RAO-004.** Optional stage list. Empty leaves `run_tool_call_loop` unchanged. Each entry has `name`, `artifact` (a path or a short conclusion), `check` (`none`, `path_exists`, or `tool_result_contains`), and `needle` for the substring check. The model claims completion with an assistant line `stage_done: <name>`. A failed check appends an observation and stays on that stage. It does not end the turn and does not run a test command. When every stage has passed, artifact pointers are appended to the assistant text already stored by the existing session write. This is not a protocol field and does not change `host_decide` or `intent_capability_route`. |
...
| `bounded_dag_live` | `false` | **VL-RAO-001.** Key retained so existing files parse. `true` does not schedule a planner or linear work nodes. Turns use `run_tool_call_loop`. The process does not rewrite the key. Not L4 emit. |
