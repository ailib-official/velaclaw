...
[INFO] ✓ Audit logging enabled (~/.config/velaclaw/audit.log)
...
$ velaclaw security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/velaclaw/audit.log
  → 47 events logged today

# Tắt sandbox tường minh (ghi vào config)
$ velaclaw config set security.sandbox.enabled false

# Bật backend cụ thể
$ velaclaw config set security.sandbox.backend firejail

# Điều chỉnh giới hạn
$ velaclaw config set security.resources.max_memory_mb 2048
```
...
```bash
$ velaclaw onboard --security-level paranoid
[1/9] Workspace Setup...
...
...
✅ **Hiển thị trạng thái** — `velaclaw security --status`
