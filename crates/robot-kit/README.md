...
A complete toolkit for building AI-powered robots with VelaClaw. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build VelaClaw with robot tools

```bash
# Clone and build
git clone https://github.com/velaclaw-labs/velaclaw
cd velaclaw
cargo build -p velaclaw-robot-kit --release
```
...
# Copy config
mkdir -p ~/.velaclaw
cp crates/robot-kit/robot.toml ~/.velaclaw/
cp crates/robot-kit/SOUL.md ~/.velaclaw/workspace/
...
# Test in mock mode
./target/release/velaclaw agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/velaclaw agent -m "Move forward 1 meter"
```
...
```rust
use velaclaw_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `velaclaw`, add a thin adapter that maps this
...
mkdir velaclaw-robot-kit
cp -r target/release/velaclaw velaclaw-robot-kit/
cp -r examples/robot_kit velaclaw-robot-kit/
cp -r ~/.velaclaw velaclaw-robot-kit/dot-velaclaw

# Include models
mkdir -p velaclaw-robot-kit/models
cp ~/.velaclaw/models/ggml-base.bin velaclaw-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf velaclaw-robot-kit.tar.gz velaclaw-robot-kit/

# Copy to USB
cp velaclaw-robot-kit.tar.gz /media/usb/TarBalls/
```
