...
2. **GitHub**: Use [GitHub Security Advisories](https://github.com/theonlyhennygod/velaclaw/security/advisories/new)
...
VelaClaw implements defense-in-depth security:
...
VelaClaw Docker images follow CIS Docker Benchmark best practices:
...
docker build -t velaclaw .
docker inspect --format='{{.Config.User}}' velaclaw
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace velaclaw gateway
```
