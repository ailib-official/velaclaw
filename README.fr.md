...
| ---------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Critique_  | Nous ne sommes **pas affiliés** à `openagen/velaclaw` ou `velaclaw.org`. Le domaine `velaclaw.org` pointe actuellement vers le fork `openagen/velaclaw`, et ce domaine/dépôt usurpe l'identité de notre site web/projet officiel.                                                                                                                                                                                 | Ne faites pas confiance aux informations, binaires, levées de fonds ou annonces provenant de ces sources. Utilisez uniquement ce dépôt et nos comptes sociaux vérifiés.                                                                                                                                                                                                                                                                                                                                                                                                          |
| 2026-02-19 | _Important_ | Nous n'avons **pas encore** lancé de site web officiel, et nous constatons des tentatives d'usurpation d'identité. Ne participez **pas** à des activités d'investissement ou de levée de fonds revendiquant le nom VelaClaw.                                                                                                                                                                                      | Utilisez ce dépôt comme source unique de vérité. Suivez [X (@velaclawlabs)](https://x.com/velaclawlabs?s=21), [Reddit (r/velaclawlabs)](https://www.reddit.com/r/velaclawlabs/), [Telegram (@velaclawlabs)](https://t.me/velaclawlabs), [Telegram CN (@velaclawlabs_cn)](https://t.me/velaclawlabs_cn), [Telegram RU (@velaclawlabs_ru)](https://t.me/velaclawlabs_ru), et [Xiaohongshu](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) pour les mises à jour officielles. |
...
### Pourquoi les équipes choisissent VelaClaw
...
## Instantané de Benchmark (VelaClaw vs OpenClaw, Reproductible)
...
- `velaclaw --help` : environ `0.02s` de temps réel, ~`3.9 Mo` d'empreinte mémoire maximale
- `velaclaw status` : environ `0.01s` de temps réel, ~`4.1 Mo` d'empreinte mémoire maximale
...
Le script `bootstrap.sh` installe Rust, clone VelaClaw, le compile, et configure votre environnement de développement initial :
...
2. Cloner le dépôt VelaClaw
3. Compiler VelaClaw en mode release
4. Installer `velaclaw` dans `~/.cargo/bin/`
5. Créer la structure de workspace par défaut dans `~/.velaclaw/workspace/`
6. Générer un fichier de configuration `~/.velaclaw/workspace/config.toml` de démarrage

Après le bootstrap, relancez votre shell ou exécutez `source ~/.cargo/env` pour utiliser la commande `velaclaw` globalement.
...
1. Configurez vos fournisseurs d'IA dans `~/.velaclaw/workspace/config.toml`
2. Consultez la [référence de configuration](docs/config-reference.md) pour les options avancées
3. Lancez l'agent : `velaclaw agent start`
4. Testez via votre canal préféré (voir [référence des canaux](docs/channels-reference.md))

## Configuration

Éditez `~/.velaclaw/workspace/config.toml` pour configurer les fournisseurs, canaux et comportement du système.
...
VelaClaw prend en charge deux backends d'exécution de code :
...
velaclaw init                # Initialise un nouveau workspace
velaclaw status              # Affiche l'état du daemon/agent
velaclaw config validate     # Vérifie la syntaxe et les valeurs de config.toml

# Gestion du daemon
velaclaw daemon start        # Démarre le daemon en arrière-plan
velaclaw daemon stop         # Arrête le daemon en cours d'exécution
velaclaw daemon restart      # Redémarre le daemon (rechargement de config)
velaclaw daemon logs         # Affiche les journaux du daemon

# Gestion de l'agent
velaclaw agent start         # Démarre l'agent (nécessite daemon en cours d'exécution)
velaclaw agent stop          # Arrête l'agent
velaclaw agent restart       # Redémarre l'agent (rechargement de config)

# Opérations de pairing
velaclaw pairing init        # Génère un nouveau secret de pairing
velaclaw pairing rotate      # Fait tourner le secret de pairing existant

# Tunneling (pour exposition publique)
velaclaw tunnel start        # Démarre un tunnel vers le daemon local
velaclaw tunnel stop         # Arrête le tunnel actif

# Diagnostic
velaclaw doctor              # Exécute les vérifications de santé du système
velaclaw version             # Affiche la version et les informations de build
```
...
enabled = true
homeserver_url = "https://matrix.org"
username = "@velaclaw:matrix.org"
password = "..."
device_name = "velaclaw-prod"
e2ee_enabled = true
```

Invitez `@velaclaw:matrix.org` dans une salle chiffrée, et le bot répondra avec le chiffrement complet. Voir [Guide Matrix E2EE](docs/matrix-e2ee-guide.md) pour la configuration de vérification de dispositif.
...
[memory]
kind = "sqlite"
path = "~/.velaclaw/workspace/memory/conversations.db"
...
[memory]
kind = "markdown"
path = "~/.velaclaw/workspace/memory/"
```
...
VelaClaw prend en charge les endpoints compatibles OpenAI :
...
VelaClaw fournit des outils intégrés pour l'exécution de code, l'accès au système de fichiers et la récupération web :
...
```bash
velaclaw daemon start
velaclaw agent start
```
...
sudo cp deployment/systemd/velaclaw-daemon.service /etc/systemd/system/
sudo cp deployment/systemd/velaclaw-agent.service /etc/systemd/system/

# Activez et démarrez les services
sudo systemctl enable velaclaw-daemon velaclaw-agent
sudo systemctl start velaclaw-daemon velaclaw-agent

# Vérifiez le statut
sudo systemctl status velaclaw-daemon
sudo systemctl status velaclaw-agent
```
...
docker run -d \
  --name velaclaw \
  -v ~/.velaclaw/workspace:/workspace \
...
VelaClaw est conçu pour fonctionner sur du matériel à faible consommation d'énergie :
...
Exposez votre daemon VelaClaw local au réseau public via des tunnels sécurisés :

```bash
velaclaw tunnel start --provider cloudflare
```
...
VelaClaw implémente plusieurs couches de sécurité :

### Pairing

Le daemon génère un secret de pairing au premier lancement stocké dans `~/.velaclaw/workspace/.pairing`. Les clients (agent, CLI) doivent présenter ce secret pour se connecter.

```bash
velaclaw pairing rotate  # Génère un nouveau secret et invalide l'ancien
```
...
VelaClaw journalise vers `~/.velaclaw/workspace/logs/` par défaut. Les journaux sont stockés par composant :

```
~/.velaclaw/workspace/logs/
...
Support de métriques Prometheus pour la surveillance en production à venir. Suivi dans [#234](https://github.com/velaclaw-labs/velaclaw/issues/234).

## Compétences (Skills)

VelaClaw prend en charge les compétences personnalisées — des modules réutilisables qui étendent les capacités du système.

### Définition de Compétence

Les compétences sont stockées dans `~/.velaclaw/workspace/skills/<nom-compétence>/` avec cette structure :
...
VelaClaw prend en charge les [Open Skills](https://github.com/openagents-com/open-skills) — un système modulaire et agnostique des fournisseurs pour étendre les capacités des agents IA.
...
Vous pouvez également surcharger au runtime avec `VELACLAW_OPEN_SKILLS_ENABLED` et `VELACLAW_OPEN_SKILLS_DIR`.
...
VelaClaw est configuré pour utiliser `rustls` pour les dépendances HTTP/TLS ; `--locked` maintient le graphe transitif déterministe sur les environnements vierges.
...
Si VelaClaw aide votre travail et que vous souhaitez soutenir le développement continu, vous pouvez faire un don ici :
...
**Ceci est le seul dépôt officiel VelaClaw :**

> <https://github.com/velaclaw-labs/velaclaw>

Tout autre dépôt, organisation, domaine ou package prétendant être "VelaClaw" ou impliquant une affiliation avec VelaClaw Labs est **non autorisé et non affilié à ce projet**. Les forks non autorisés connus seront listés dans [TRADEMARK.md](TRADEMARK.md).

Si vous rencontrez une usurpation d'identité ou une utilisation abusive de marque, veuillez [ouvrir une issue](https://github.com/velaclaw-labs/velaclaw/issues).

---

## Licence

VelaClaw est sous double licence pour une ouverture maximale et la protection des contributeurs :
...
Le nom **VelaClaw** et le logo sont des marques déposées de VelaClaw Labs. Cette licence n'accorde pas la permission de les utiliser pour impliquer une approbation ou une affiliation. Voir [TRADEMARK.md](TRADEMARK.md) pour les utilisations permises et interdites.
...
- Nouvelle `Skill` → `~/.velaclaw/workspace/skills/<n>/`

---

**VelaClaw** — Zéro surcharge. Zéro compromis. Déployez n'importe où. Échangez n'importe quoi. 🦀

## Historique des Étoiles

<p align="center">
  <a href="https://www.star-history.com/#velaclaw-labs/velaclaw&type=date&legend=top-left">
    <picture>
     <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=velaclaw-labs/velaclaw&type=date&theme=dark&legend=top-left" />
     <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=velaclaw-labs/velaclaw&type=date&legend=top-left" />
     <img alt="Graphique Historique des Étoiles" src="https://api.star-history.com/svg?repos=velaclaw-labs/velaclaw&type=date&legend=top-left" />
    </picture>
  </a>
</p>
