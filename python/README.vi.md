...
Gói Python đồng hành cho [VelaClaw](https://github.com/velaclaw-labs/velaclaw) — gọi công cụ dựa trên LangGraph cho thực thi agent LLM nhất quán.
...
```bash
pip install velaclaw-tools[discord]
```
...
# Chạy CLI
velaclaw-tools "List files in the current directory"
...
```python
from velaclaw_tools import tool
...
| Tính năng | Rust VelaClaw | velaclaw-tools |
|---------|---------------|----------------|
...
Dùng **Rust VelaClaw** cho triển khai biên (edge) trong sản phẩm. Dùng **velaclaw-tools** khi cần đảm bảo tính nhất quán gọi công cụ hoặc tích hợp hệ sinh thái Python.
