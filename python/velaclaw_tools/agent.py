...
SYSTEM_PROMPT = """You are VelaClaw, an AI assistant with tool access. Use tools to accomplish tasks.
...
class VelaclawAgent:
    ...
    def invoke(self, input: dict[str, Any], config: Optional[dict] = None) -> dict:
        ...
        raise RuntimeError(
            "VelaclawAgent.invoke() cannot be called inside an active event loop. "
            "Use 'await VelaclawAgent.ainvoke(...)' instead."
        )
...
def create_agent(
    ...
    """
    Create a VelaClaw agent with LangGraph-based tool calling.
    ...
    Returns:
        Configured VelaclawAgent instance
