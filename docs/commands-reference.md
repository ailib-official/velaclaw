...
- `velaclaw doctor host-decide [--message <text>] [--tag <Tag>] [--force] [--set-override provider/model] [--clear-override] [--session-key <id>]` — **ORCH-HOST-001** observe: CAP reachable ∩ host Decide ranking (reason + disclaimer). Independent of `[agent].host_decide` when `--force` is set. Session override is process-local and must remain in reachable set. Requires `--features ai-protocol`.
