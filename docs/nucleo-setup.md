# VelaClaw on Nucleo-F401RE — Step-by-Step Guide

Run VelaClaw on your Mac or Linux host. Connect a Nucleo-F401RE via USB. Control GPIO (LED, pins) via Telegram or CLI.

---

## Get Board Info via Telegram (No Firmware Needed)

VelaClaw can read chip info from the Nucleo over USB **without flashing any firmware**. Message your Telegram bot:
...
VelaClaw includes everything for Nucleo-F401RE:
...
| Flash command | `velaclaw peripheral flash-nucleo` | Builds firmware, flashes via probe-rs |
...
### 1.2 Flash via VelaClaw

From the velaclaw repo root:

```bash
velaclaw peripheral flash-nucleo
```

This builds `firmware/velaclaw-nucleo` and runs `probe-rs run --chip STM32F401RETx`. The firmware runs immediately after flashing.
...
| 5 | `velaclaw daemon` or `velaclaw agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `velaclaw peripheral list` to verify.
