...
fn strip_tool_call_tags(message: &str) -> String {
    crate::util::strip_tool_call_markup(message)
}
...
impl TelegramChannel {
    ...
    async fn load_config_without_env() -> anyhow::Result<Config> {
        ...
        config.workspace_dir = velaclaw_dir.join("workspace");
        Ok(config)
    }
    ...
    async fn persist_allowed_identity(&self, identity: &str) -> anyhow::Result<()> {
        ...
        let Some(telegram) = config.channels_config.telegram.as_mut() else {
            anyhow::bail!(
                ...
                or run `velaclaw onboard --channels-only` to configure interactively"
            );
        };
                                ...
                                Ok(()) => {
                                    let _ = self
                                        .send(&SendMessage::new(
                                            "✅ Telegram account bound successfully. You can talk to VelaClaw now.",
                                            &chat_id,
                                        ))
                                        .await;
                                    tracing::info!(
...
Ensure only one `velaclaw` process is using this bot token."
                    );
...
mod tests {
    ...
    fn telegram_extract_bind_code_supports_bot_mention() {
        assert_eq!(
            TelegramChannel::extract_bind_code("/bind@velaclaw_bot 654321"),
            Some("654321")
        );
    }
