...
impl PrometheusObserver {
    pub fn new() -> Self {
        ...
        let agent_starts = IntCounterVec::new(
            prometheus::Opts::new("zerospider_agent_starts_total", "Total agent invocations"),
            &["provider", "model"],
        )
        .expect("valid metric");

        let tool_calls = IntCounterVec::new(
            prometheus::Opts::new("zerospider_tool_calls_total", "Total tool calls"),
            &["tool", "success"],
        )
        .expect("valid metric");
        ...
        let channel_messages = IntCounterVec::new(
            prometheus::Opts::new(
                "zerospider_channel_messages_total",
        ...
        let heartbeat_ticks = prometheus::IntCounter::new(
            "zerospider_heartbeat_ticks_total",
            "Total heartbeat ticks",
        )
        .expect("valid metric");

        let errors = IntCounterVec::new(
            prometheus::Opts::new("zerospider_errors_total", "Total errors by component"),
            &["component"],
        )
        .expect("valid metric");
        ...
        let agent_duration = HistogramVec::new(
            HistogramOpts::new(
                "zerospider_agent_duration_seconds",
        ...
        let tool_duration = HistogramVec::new(
            HistogramOpts::new(
                "zerospider_tool_duration_seconds",
        ...
        let request_latency = Histogram::with_opts(
            HistogramOpts::new(
                "zerospider_request_latency_seconds",
        ...
        let tokens_used = prometheus::IntGauge::new(
            "zerospider_tokens_used_last",
            "Tokens used in the last request",
        )
        .expect("valid metric");

        let active_sessions = GaugeVec::new(
            prometheus::Opts::new("zerospider_active_sessions", "Number of active sessions"),
            &[],
        )
        .expect("valid metric");

        let queue_depth = GaugeVec::new(
            prometheus::Opts::new("zerospider_queue_depth", "Message queue depth"),
            &[],
        )
        .expect("valid metric");
...
mod tests {
    ...
    fn encode_produces_prometheus_text_format() {
        ...
        assert!(output.contains("zerospider_agent_starts_total"));
        assert!(output.contains("zerospider_tool_calls_total"));
        assert!(output.contains("zerospider_heartbeat_ticks_total"));
        assert!(output.contains("zerospider_request_latency_seconds"));
    }
    ...
    fn counters_increment_correctly() {
        ...
        let output = obs.encode();
        assert!(output.contains("zerospider_heartbeat_ticks_total 3"));
    }
    ...
    fn tool_calls_track_success_and_failure_separately() {
        ...
        let output = obs.encode();
        assert!(output.contains(r#"zerospider_tool_calls_total{success="true",tool="shell"} 2"#));
        assert!(output.contains(r#"zerospider_tool_calls_total{success="false",tool="shell"} 1"#));
    }
    ...
    fn errors_track_by_component() {
        ...
        let output = obs.encode();
        assert!(output.contains(r#"zerospider_errors_total{component="provider"} 2"#));
        assert!(output.contains(r#"zerospider_errors_total{component="channels"} 1"#));
    }
    ...
    fn gauge_reflects_latest_value() {
        ...
        let output = obs.encode();
        assert!(output.contains("zerospider_tokens_used_last 200"));
    }
}
