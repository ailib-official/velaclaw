# Triển khai mạng — VelaClaw trên Raspberry Pi và mạng nội bộ

Tài liệu này hướng dẫn triển khai VelaClaw trên Raspberry Pi hoặc host khác trong mạng nội bộ, với các channel Telegram và webhook tùy chọn.
...
|------|----------------------|----------|
| **Telegram polling** | Không | VelaClaw poll Telegram API; hoạt động từ bất kỳ đâu |
| **Matrix sync (kể cả E2EE)** | Không | VelaClaw sync qua Matrix client API; không cần webhook đến |
...
**Lưu ý:** Telegram, Discord và Slack dùng **long-polling** — VelaClaw thực hiện các request ra ngoài. Không cần port forwarding hoặc public IP.

---

## 2. VelaClaw trên Raspberry Pi
...
- Channel Telegram hoạt động: VelaClaw poll Telegram API (outbound)
...
2. Khởi động tunnel:
   ...
   ```
   Hoặc dùng `velaclaw tunnel` (xem tài liệu tunnel).

3. VelaClaw sẽ từ chối `0.0.0.0` trừ khi `allow_public_bind = true` hoặc có tunnel đang hoạt động.
...
- VelaClaw gọi `https://api.telegram.org/bot{token}/getUpdates`
...
Chạy `velaclaw daemon` — channel Telegram khởi động tự động.
...
- Chỉ chạy một instance runtime cho cùng token (khuyến nghị: service `velaclaw daemon`).
...
- [ ] Chạy `velaclaw daemon --host 127.0.0.1 --port 3000` (Telegram hoạt động không cần 0.0.0.0)
