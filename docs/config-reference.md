...
| `bounded_dag_live` | `false` | **VL-NA-011/015 (opt-in).** When true, CLI and Web run a **tool-free planner chat** (session default / picker model) then linear work nodes on the existing tool loop. Plan always replans. Build with a short approval reuses `dag_plan:<session>`; any other Build message clears plan + `dag_art:*` and replans. Work nodes select Contact (`hint:`) and **ignore** the session picker. Default off. Not L4 `candidate_dag_emit`. Channels unchanged. Requires `--features ai-protocol`. |
