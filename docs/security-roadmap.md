# VelaClaw Security Improvement Roadmap
...
VelaClaw already has **excellent application-layer security**:
...
## Comparison: VelaClaw vs PicoClaw vs Production Grade

| Feature | PicoClaw | VelaClaw Now | VelaClaw + Roadmap | Production Target |
|---------|----------|--------------|-------------------|-------------------|
...
| Security self-test (`velaclaw audit --check`) | 1 day | Low |
...
[security.audit]
enabled = true
log_path = "~/.config/velaclaw/audit.log"
sign_events = true
max_size_mb = 100
...
velaclaw security --check
# → ✓ Sandbox: Firejail active
# → ✓ Audit logging enabled (42 events today)
# → → Resource limits: 512MB mem, 50% CPU

# Audit log queries
velaclaw audit --user @alice --since 24h
velaclaw audit --risk high --violations-only
velaclaw audit --verify-signatures

# Sandbox test
velaclaw sandbox --test
# → Testing isolation...
...
**VelaClaw is already more secure than PicoClaw** with:
...
**By implementing this roadmap**, VelaClaw becomes:
...
**Value**: Transforms VelaClaw from "safe for testing" to "safe for production"
