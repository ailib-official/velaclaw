# Thêm Board và Tool — Hướng dẫn phần cứng VelaClaw

Hướng dẫn này giải thích cách thêm board phần cứng mới và tool tùy chỉnh vào VelaClaw.

## Bắt đầu nhanh: Thêm board qua CLI

```bash
# Thêm board (cập nhật ~/.velaclaw/config.toml)
velaclaw peripheral add nucleo-f401re /dev/ttyACM0
velaclaw peripheral add arduino-uno /dev/cu.usbmodem12345
velaclaw peripheral add rpi-gpio native   # cho Raspberry Pi GPIO (Linux)
...
Với feature `rag-pdf`, VelaClaw có thể lập chỉ mục file PDF:
...
2. **Thêm vào config** — `velaclaw peripheral add my-board /dev/ttyUSB0`
...
| Lệnh | Mô tả |
|------|-------|
| `velaclaw peripheral list` | Liệt kê các board đã cấu hình |
| `velaclaw peripheral add <board> <path>` | Thêm board (ghi vào config) |
| `velaclaw peripheral flash` | Nạp firmware Arduino |
| `velaclaw peripheral flash-nucleo` | Nạp firmware Nucleo |
| `velaclaw hardware discover` | Liệt kê thiết bị USB |
| `velaclaw hardware info` | Thông tin chip qua probe-rs |
