//! Flash VelaClaw Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash VelaClaw Nucleo firmware. Builds from firmware/velaclaw-nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (velaclaw's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("velaclaw-nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from velaclaw repo root.",
            firmware_dir.display()
        );
    }

    println!("Building VelaClaw Nucleo firmware...");
    ...
    println!("VelaClaw Nucleo firmware flashed successfully.");
