# Adding Boards and Tools — VelaClaw Hardware Guide

This guide explains how to add new hardware boards and custom tools to VelaClaw.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.velaclaw/config.toml)
velaclaw peripheral add nucleo-f401re /dev/ttyACM0
velaclaw peripheral add arduino-uno /dev/cu.usbmodem12345
velaclaw peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, VelaClaw can index PDF files:
...
2. **Add to config** — `velaclaw peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `velaclaw peripheral list` | List configured boards |
| `velaclaw peripheral add <board> <path>` | Add board (writes config) |
| `velaclaw peripheral flash` | Flash Arduino firmware |
| `velaclaw peripheral flash-nucleo` | Flash Nucleo firmware |
| `velaclaw hardware discover` | List USB devices |
| `velaclaw hardware info` | Chip info via probe-rs |
