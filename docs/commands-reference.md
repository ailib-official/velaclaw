...
`[agent].bounded_dag_live` is a **separate** opt-in (default `false`): CLI + Web only. Empty path = planner node (session default model) then linear work nodes; envelope budget follows **that node's Contact model** window; work-node system is a small P0+P1 pack. Not L4 `candidate_dag_emit`. See [config-reference.md](config-reference.md#agent).
