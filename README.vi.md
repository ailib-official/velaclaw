...
|---|---|---|---|
| 2026-02-19 | _Nghiêm trọng_ | Chúng tôi **không có liên kết** với `openagen/velaclaw` hoặc `velaclaw.org`. Tên miền `velaclaw.org` hiện đang trỏ đến fork `openagen/velaclaw`, và tên miền/repository đó đang mạo danh website/dự án chính thức của chúng tôi. | Không tin tưởng thông tin, binary, gây quỹ, hay thông báo từ các nguồn đó. Chỉ sử dụng repository này và các tài khoản mạng xã hội đã được xác minh của chúng tôi. |
| 2026-02-19 | _Quan trọng_ | Chúng tôi **chưa** ra mắt website chính thức, và chúng tôi đang ghi nhận các nỗ lực mạo danh. **Không** tham gia bất kỳ hoạt động đầu tư hoặc gây quỹ nào tuyên bố mang tên VelaClaw. | Sử dụng repository này làm nguồn thông tin duy nhất đáng tin cậy. Theo dõi [X (@velaclawlabs)](https://x.com/velaclawlabs?s=21), [Reddit (r/velaclawlabs)](https://www.reddit.com/r/velaclawlabs/), [Telegram (@velaclawlabs)](https://t.me/velaclawlabs), [Telegram CN (@velaclawlabs_cn)](https://t.me/velaclawlabs_cn), [Telegram RU (@velaclawlabs_ru)](https://t.me/velaclawlabs_ru), và [Xiaohongshu](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) để nhận cập nhật chính thức. |
...
## So sánh hiệu suất (VelaClaw vs OpenClaw, có thể tái tạo)
...
- `velaclaw --help`: khoảng `0.02s`, bộ nhớ đỉnh ~`3.9MB`
- `velaclaw status`: khoảng `0.01s`, bộ nhớ đỉnh ~`4.1MB`
...
Hoặc bỏ qua các bước trên, cài hết mọi thứ (system deps, Rust, VelaClaw) chỉ bằng một lệnh:

```bash
curl -LsSf https://raw.githubusercontent.com/velaclaw-labs/velaclaw/main/scripts/install.sh | bash
```
...
VELACLAW_CONTAINER_CLI=podman ./bootstrap.sh --docker
...
<https://github.com/velaclaw-labs/velaclaw/releases/latest>

Ví dụ (ARM64 Linux):

```bash
curl -fsSLO https://github.com/velaclaw-labs/velaclaw/releases/latest/download/velaclaw-aarch64-unknown-linux-gnu.tar.gz
tar xzf velaclaw-aarch64-unknown-linux-gnu.tar.gz
install -m 0755 velaclaw "$HOME/.cargo/bin/velaclaw"
```
...
velaclaw gateway                # mặc định: 127.0.0.1:3000
velaclaw gateway --port 0       # cổng ngẫu nhiên (tăng cường bảo mật)
...
velaclaw integrations info Telegram
...
VelaClaw hỗ trợ profile xác thực theo gói đăng ký (đa tài khoản, mã hóa khi lưu).

- File lưu trữ: `~/.velaclaw/auth-profiles.json`
- Khóa mã hóa: `~/.velaclaw/.secret_key`
...
<p align="center">
  <img src="docs/architecture.svg" alt="VelaClaw Architecture" width="900" />
</p>

| Hệ thống con | Trait | Đi kèm sẵn | Mở rộng |
|-----------|-------|------------|--------|
| **Mô hình AI** | `Provider` | Danh mục provider qua `velaclaw providers` (ai-protocol `provider/model`) | `openai/gpt-5.2` |
...
Khi cấu hình `runtime.kind` không được hỗ trợ, VelaClaw sẽ thoát với thông báo lỗi rõ ràng thay vì âm thầm fallback về native.
...
# Khi provider = "postgres", VelaClaw dùng PostgreSQL để lưu memory.
# Khóa db_url cũng chấp nhận alias `dbURL` để tương thích ngược.
#
# [storage.provider.config]
# provider = "postgres"
# db_url = "postgres://user:password@host:5432/velaclaw"
# schema = "public"
# table = "memories"
...
# VELACLAW_LUCID_CMD=/usr/local/bin/lucid            # mặc định: lucid
# VELACLAW_LUCID_BUDGET=200                          # mặc định: 200
# VELACLAW_LUCID_LOCAL_HIT_THRESHOLD=3               # số lần hit cục bộ để bỏ qua external recall
# VELACLAW_LUCID_RECALL_TIMEOUT_MS=120               # giới hạn thời gian cho lucid context recall
# VELACLAW_LUCID_STORE_TIMEOUT_MS=800                # timeout đồng bộ async cho lucid store
# VELACLAW_LUCID_FAILURE_COOLDOWN_MS=15000           # thời gian nghỉ sau lỗi lucid, tránh thử lại liên tục
```

## Bảo mật

VelaClaw thực thi bảo mật ở **mọi lớp** — không chỉ sandbox. Đáp ứng tất cả các hạng mục trong danh sách kiểm tra bảo mật của cộng đồng.
...
> **Tự chạy nmap:** `nmap -p 1-65535 <your-host>` — VelaClaw chỉ bind vào localhost, nên không có gì bị lộ ra ngoài trừ khi bạn cấu hình tunnel tường minh.
...
Với các phản hồi không phải văn bản, VelaClaw có thể gửi file đính kèm Telegram khi assistant bao gồm các marker:
...
VelaClaw hỗ trợ hai backend WhatsApp:
...
2. **Cấu hình VelaClaw:**
   ...
   session_path = "~/.velaclaw/state/whatsapp-web/session.db"
...
3. **Khởi động channel/daemon và liên kết thiết bị:**
   - Chạy `velaclaw channel start` (hoặc `velaclaw daemon`).
...
6. **Kiểm tra:** Gửi tin nhắn đến số WhatsApp Business của bạn — VelaClaw sẽ phản hồi qua LLM.

## Cấu hình

Config: `~/.velaclaw/config.toml` (được tạo bởi `onboard`)

Khi `velaclaw channel start` đang chạy, các thay đổi với `default_provider`,
...
#   "metadata": {"session_name": "...", "source": "velaclaw.browser", "version": "..."}
# }
...
VelaClaw dùng một khóa provider (`ollama`) cho cả triển khai Ollama cục bộ và từ xa:
...
## Gói Python đi kèm (`velaclaw-tools`)

Với các LLM provider có tool calling native không ổn định (ví dụ: GLM-5/Zhipu), VelaClaw đi kèm gói Python dùng **LangGraph để gọi tool** nhằm đảm bảo tính nhất quán:

```bash
pip install velaclaw-tools
```

```python
from velaclaw_tools import create_agent, shell, file_read
...
VelaClaw hỗ trợ persona AI **không phụ thuộc nền tảng** qua hai định dạng:
...
[AIEOS](https://aieos.org) là framework chuẩn hóa cho định danh AI di động. VelaClaw hỗ trợ payload AIEOS v1.1 JSON, cho phép bạn:
...
VelaClaw chấp nhận cả payload AIEOS đầy đủ lẫn dạng rút gọn, rồi chuẩn hóa về một định dạng system prompt thống nhất.
...
Bạn cũng có thể ghi đè lúc runtime với `VELACLAW_OPEN_SKILLS_ENABLED` và `VELACLAW_OPEN_SKILLS_DIR`.
...
VelaClaw được cấu hình để dùng `rustls` cho các dependencies HTTP/TLS; `--locked` giữ cho dependency graph nhất quán trên các môi trường mới.
...
Nếu VelaClaw giúp ích cho công việc của bạn và bạn muốn hỗ trợ phát triển liên tục, bạn có thể quyên góp tại đây:
...
**Đây là repository VelaClaw chính thức duy nhất:**
> https://github.com/velaclaw-labs/velaclaw

Bất kỳ repository, tổ chức, tên miền hay gói nào khác tuyên bố là "VelaClaw" hoặc ngụ ý liên kết với VelaClaw Labs đều là **không được ủy quyền và không liên kết với dự án này**. Các fork không được ủy quyền đã biết sẽ được liệt kê trong [TRADEMARK.md](TRADEMARK.md).

Nếu bạn phát hiện hành vi mạo danh hoặc lạm dụng nhãn hiệu, vui lòng [mở một issue](https://github.com/velaclaw-labs/velaclaw/issues).

---

## Giấy phép

VelaClaw được cấp phép kép để tối đa hóa tính mở và bảo vệ người đóng góp:
...
Tên **VelaClaw** và logo là nhãn hiệu của VelaClaw Labs. Giấy phép này không cấp phép sử dụng chúng để ngụ ý chứng thực hoặc liên kết. Xem [TRADEMARK.md](TRADEMARK.md) để biết các sử dụng được phép và bị cấm.
...
- `Skill` mới → `~/.velaclaw/workspace/skills/<name>/`

---

**VelaClaw** — Không tốn thêm tài nguyên. Không đánh đổi. Triển khai ở đâu cũng được. Thay thế gì cũng được. 🦀
...
<p align="center">
  <a href="https://www.star-history.com/#velaclaw-labs/velaclaw&type=date&legend=top-left">
    <picture>
     ...
     <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=velaclaw-labs/velaclaw&type=date&legend=top-left" />
    </picture>
  </a>
</p>
