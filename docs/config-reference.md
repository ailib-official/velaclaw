...
| `bounded_dag_live` | `false` | **VL-NA-011/015/020/025 (opt-in).** Multi-step user asks: planner then linear work nodes on the existing tool loop. Greetings and single-shot Q&A skip the planner (session default model, one chat). Work nodes: Contact `hint:` (session picker ignored); envelope budget is that hop's protocol `context_window` (`hint:` resolved via `[[model_routes]]`); product system is P0+P1 (no bootstrap bodies, tool names not JSON schemas). HardBudget stays fail-closed and names hop model + window. Default off. Not L4 `candidate_dag_emit`. |
