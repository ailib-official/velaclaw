...
1. **Explicit user pick** — CLI `-p` / `--model`, Web `model_id`, or process session override (must be CAP-reachable when the index is available; otherwise fail closed). Session-tombstoned ids (HTTP 410/404/402) are skipped (VL-APE-010 / I10).
2. **`host_decide`** (if enabled) — CAP reachable ∩ embedded CostRouter-shaped pricing. **Work / chat_only / parlor hops skip this** so `optimize=cost` cannot steal the work Route (I9/I10). Planner/judge hops may still use it.
...
DAG-related keys below: **`template_dag` / candidate emit / shadow** stay library/doctor (AI-DAG frozen off the default turn). **`bounded_dag_live` is a separate opt-in** for a handwritten linear L2 graph on CLI + Web (not L4 emit). On Web, an empty-I hop opens a text prompt for one allowed command (`ssh <alias> …` only when that alias is in `deploy.servers`); filling it continues the same turn. CLI without HITL keeps the short Ask. This does not widen the default allowlist. Planned `tool_direct` I is one admit-safe invoke: a simple argv (flags allowed), `ssh [client-opts] <alias> <that argv>`, or a pipe of those segments. Command lists, `sh -c`/`bash -c`, and `xargs` stay rejected. Admit rejects write `admit_reject.json` under graph scratch and do not execute. Empty command stdout stays a stored record and later hops see `(no output)` for that node id. A missing tool record stops and names the node. An `llm_cognition` node receives no tools.
...
| `bounded_dag_live` | `false` | **VL-NA-011/015/020/025/026/028/029/030 + VL-APE-001…005 / VL-APE-010 / VL-APE-020 (opt-in).** First hop is in-band `chat_only`, `single_work` (one native tool loop), or a DAG (linear or `fork` ready-set). Invalid / empty `chat_only` is `single_work`. A 1-node non-ToolDirect graph collapses to `single_work`; 2+ nodes or a 1-node ToolDirect stay Plan. No 1-node split-refine. Never silent `chat_only`. Planner uses `fast` route or session cheap default (not the picker); **work hops use the Web picker** when the request has `provider/model` via Route(C, Context) (VL-APE-002) unless that id is session-tombstoned. Non-session capabilities (embed, …) use capability hints, not the picker. Work hops are not CostRouter. Successful hops skip observe (VL-APE-001); CLI and Web share `graph_scheduler`. LLM hops default to native `tool_calls` (no user-shape tool results); tool-only nodes invoke `execute_tool_batch` (VL-APE-003). Independent tool nodes may run as one ready-set batch (VL-APE-004). MS-APE-R1 budget tests lock `chat_only==1`, zero observe on successful hops, parlor ≤1 (VL-APE-005). Cap/PolicyDeny still skip observe (VL-NA-045). Session turns always run `prepare_turn_history` before the first hop (VL-CTX-001); VL-NA-019 skip is Plan-phase preview only. Dist default off. Not L4 `candidate_dag_emit`. |
...
| `dag_fail_auto_replan` | `false` | **VL-NA-024 / VL-APE-004 (opt-in).** After a live DAG work-node fail, retry that node once in the same turn (unavailable/quota → session default; policy/other same Contact; transport stops). Typed fail stores a fail cursor that **freezes the completed prefix**; the next user turn may run one A repair (`splice_remaining_plan` only on that fail path). Successful hops never splice remaining. Dist default off. |
...
- `auto` (default): resolve via provider manifest `tool_calling` and `ExecutionHandle::tool_calling_policy()`. When the provider supports native tools and the manifest strategy prefers native (e.g. DeepSeek hybrid), `NativeToolDispatcher` is used; otherwise `XmlToolDispatcher`. **Live DAG LLM hops (VL-APE-003):** `auto`/`native` fail closed if the provider has no native `tool_calls` (no silent XML / user-shape). Explicit `xml` remains node-level compat; tool results still use role=`tool` on the live path.
