...
Quota/EOL notices name the **executed** logical id (`executed model \`…\``); the Web picker is a second field only when it differs. The live DAG planner hop uses the `fast` `[[model_routes]]` id (else session default), not the picker.
