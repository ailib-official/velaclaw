...
| `bounded_dag_live` | `false` | **VL-NA-011/015/020/025/026/028 (opt-in).** First hop is in-band `chat_only` or a 1–8 node linear DAG. Tools always run as DAG nodes (a `single_work` label or invalid JSON becomes a 1-node graph, then one split-refine chat). Never silent `chat_only`. Work hops use the session default model (not CostRouter). After each hop, observe may `replan_remaining` once. Dist default off. Not L4 `candidate_dag_emit`. |
