...
- **GitHub CLI:** put `GH_TOKEN` or `GITHUB_TOKEN` in the daemon `EnvironmentFile` (`daemon.env`, mode `600`). Shell children inherit those two names after `env_clear`. Do not `cat` PAT files into chat. Landlock still cannot read `~/.config/gh`.
