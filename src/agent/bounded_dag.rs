...
/// Node order for live/preview: linear chain, or Kahn topo when any node has `fork`.
pub fn schedule_node_ids(dag: &DagManifest) -> Result<Vec<String>> {
    if dag.nodes.iter().any(|n| !n.fork.is_empty()) {
        topo_node_ids(dag)
    } else {
        linear_node_ids(dag)
    }
...
/// Kahn topo covering every node. `entry` is scheduled first among indegree-0 nodes.
pub fn topo_node_ids(dag: &DagManifest) -> Result<Vec<String>> {
    ...
    let mut indeg: HashMap<&str, usize> =
        dag.nodes.iter().map(|n| (n.id.as_str(), 0usize)).collect();
    for node in &dag.nodes {
        for suc in node.successors() {
            if !by_id.contains_key(suc) {
                bail!(
                    "bounded DAG '{}': missing successor '{}' from '{}'",
                    dag.id,
                    suc,
                    node.id
                );
            }
            *indeg.entry(suc).or_insert(0) += 1;
        }
    }
    let mut ready: Vec<&str> = dag
        ...
        .collect();
    ready.sort_by_key(|id| if *id == dag.entry.as_str() { 0 } else { 1 });
    let mut order = Vec::new();
    let mut steps = 0u32;
    while let Some(id) = ready.first().copied() {
        if steps >= dag.max_steps {
            bail!(
                "bounded DAG '{}': topo walk exceeded max_steps={}",
                dag.id,
                dag.max_steps
            );
        }
        ...
        let Some(node) = by_id.get(id) else {
            bail!("bounded DAG '{}': missing node '{}'", dag.id, id);
        };
        for suc in node.successors() {
            let deg = indeg.get_mut(suc).expect("indeg suc");
            *deg = deg.saturating_sub(1);
            if *deg == 0 && !order.iter().any(|o| o == suc) && !ready.contains(&suc) {
                ready.push(suc);
            }
        }
    }
    if order.len() != dag.nodes.len() {
        bail!(
            "bounded DAG '{}': not a DAG cover (walked {} of {} nodes)",
            dag.id,
            order.len(),
            dag.nodes.len()
        );
...
mod tests {
    ...
    #[test]
    fn fork_graph_topo_covers_independent_nodes() {
        let json = r#"{
          "schema_version": "0.1.0",
          "id": "fork",
          "entry": "start",
          "max_steps": 8,
          "nodes": [
            {"id":"start","task_type":"t","model_selector":{"capabilities":["coding"]},"next":"a","fork":["b"]},
            {"id":"a","task_type":"t","model_selector":{"capabilities":["coding"]},"next":"join"},
            {"id":"b","task_type":"t","model_selector":{"capabilities":["coding"]},"next":"join"},
            {"id":"join","task_type":"t","model_selector":{"capabilities":["coding"]},"next":null}
          ]
        }"#;
        let dag = parse_dag_json(json).unwrap();
        assert!(linear_node_ids(&dag).is_err());
        let ids = schedule_node_ids(&dag).unwrap();
        assert_eq!(ids[0], "start");
        assert!(ids.contains(&"a".to_string()) && ids.contains(&"b".to_string()));
        assert_eq!(ids.last().map(String::as_str), Some("join"));
        assert!(
            ids.iter().position(|x| x == "a").unwrap()
                < ids.iter().position(|x| x == "join").unwrap()
        );
        assert!(
