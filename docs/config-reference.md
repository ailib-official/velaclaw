...
| `bounded_dag_live` | `false` | **VL-NA-011/015/020/025/026/028/029/030 (opt-in).** First hop is in-band `chat_only` or a 1–8 node linear DAG. Tools always run as DAG nodes (a `single_work` label or invalid JSON becomes a 1-node graph, then one split-refine chat). Never silent `chat_only`. Planner/observe keep the session default model; **work hops use the Web picker** when the request has `provider/model` (VL-NA-043/045). Work hops are not CostRouter. After each hop, observe may `replan_remaining` once unless the hop closed Cap/PolicyDeny (VL-NA-045; 15s timeout fail-open). Session turns always run `prepare_turn_history` before the first hop (VL-CTX-001); VL-NA-019 skip is Plan-phase preview only. Dist default off. Not L4 `candidate_dag_emit`. |
