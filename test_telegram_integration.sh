#!/bin/bash
# VelaClaw Telegram Integration Test Suite
...
if [ -f "target/release/velaclaw" ]; then
    BINARY_SIZE=$(ls -lh target/release/velaclaw | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/velaclaw 2>/dev/null || stat -c%s target/release/velaclaw)
...
CONFIG_PATH="$HOME/.velaclaw/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'velaclaw onboard' first"
    fi
else
    warn "No config file found - run 'velaclaw onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/velaclaw channel doctor 2>&1 || true)
END_TIME=$(date +%s)
