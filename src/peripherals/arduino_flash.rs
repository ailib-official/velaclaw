//! Flash VelaClaw Arduino firmware via arduino-cli.
//!
...
/// VelaClaw Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
const FIRMWARE_INO: &str = include_str!("../../firmware/velaclaw-arduino/velaclaw-arduino.ino");

const FQBN: &str = "arduino:avr:uno";
const SKETCH_NAME: &str = "velaclaw-arduino";
...
/// Flash VelaClaw firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("velaclaw_flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling VelaClaw Arduino firmware...");
    ...
    println!("VelaClaw firmware flashed successfully.");
