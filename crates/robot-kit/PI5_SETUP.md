...
Complete guide to setting up a VelaClaw-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.velaclaw/models
cp models/ggml-base.bin ~/.velaclaw/models/
```
...
mkdir -p ~/.velaclaw/models/piper
cd ~/.velaclaw/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.velaclaw/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
### 7. Build VelaClaw Robot Kit

```bash
# Clone repo (or copy from USB)
git clone https://github.com/theonlyhennygod/velaclaw
cd velaclaw

# Build robot kit
cargo build --release -p velaclaw-robot-kit

# Build main velaclaw (optional, if using as agent)
cargo build --release
```
...
```bash
mkdir -p ~/.velaclaw
nano ~/.velaclaw/robot.toml
```

```toml
# ~/.velaclaw/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.velaclaw/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/velaclaw_sensors.fifo"
...
### Start VelaClaw Agent

```bash
# Configure VelaClaw to use robot tools
cat > ~/.velaclaw/config.toml << 'EOF'
...
cp ~/velaclaw/crates/robot-kit/SOUL.md ~/.velaclaw/workspace/

# Start agent
./target/release/velaclaw agent
```
...
# Start sensor loop
if [ ! -p /tmp/velaclaw_sensors.fifo ]; then
    mkfifo /tmp/velaclaw_sensors.fifo
fi
...
# Start velaclaw
cd ~/velaclaw
./target/release/velaclaw daemon &
AGENT_PID=$!
...
```bash
# /etc/systemd/system/velaclaw-robot.service
sudo tee /etc/systemd/system/velaclaw-robot.service << 'EOF'
[Unit]
Description=VelaClaw Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/velaclaw
...
sudo systemctl enable velaclaw-robot
sudo systemctl start velaclaw-robot

# Check status
sudo systemctl status velaclaw-robot
journalctl -u velaclaw-robot -f  # View logs
```
