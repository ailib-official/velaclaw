# CLAUDE.md — VelaClaw Agent Engineering Protocol
...
VelaClaw is a Rust-first autonomous agent runtime optimized for:
...
- If identity-like naming is required in tests/examples, use VelaClaw-native labels only (`VelaClawAgent`, `velaclaw_user`, `velaclaw_node`).
...
- If identity-like context is unavoidable, use VelaClaw-scoped roles/labels only (for example: `VelaClawAgent`, `VelaClawOperator`, `velaclaw_user`) and avoid real-world personas.
- Recommended identity-safe naming palette (use when identity-like context is required):
    - actor labels: `VelaClawAgent`, `VelaClawOperator`, `VelaClawMaintainer`, `velaclaw_user`
    - service/runtime labels: `velaclaw_bot`, `velaclaw_service`, `velaclaw_runtime`, `velaclaw_node`
    - environment labels: `velaclaw_project`, `velaclaw_workspace`, `velaclaw_channel`
