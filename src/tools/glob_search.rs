...
mod tests {
    ...
    async fn glob_search_single_file() {
        ...
        let result = tool.execute(json!({"pattern": "hello.txt"}), &ToolExecutionContext::default()).await.unwrap();
    ...
    async fn glob_search_rejects_absolute_path() {
        let tool = GlobSearchTool::new(test_security(std::env::temp_dir()));
        let result = tool.execute(json!({"pattern": "/etc/**/*"}), &ToolExecutionContext::default()).await.unwrap();
    ...
    async fn glob_search_rejects_dotdot_only() {
        let tool = GlobSearchTool::new(test_security(std::env::temp_dir()));
        let result = tool.execute(json!({"pattern": ".."}), &ToolExecutionContext::default()).await.unwrap();
    ...
    async fn glob_search_includes_workspace_symlink_directory() {
        ...
        let result = tool
            .execute(json!({"pattern": "ai-lib-plans/tools/hk_vps*.sh"}), &ToolExecutionContext::default())
            .await
            .unwrap();
    ...
    async fn glob_search_invalid_pattern() {
        ...
        let tool = GlobSearchTool::new(test_security(dir.path().to_path_buf()));
        let result = tool.execute(json!({"pattern": "[invalid"}), &ToolExecutionContext::default()).await.unwrap();
