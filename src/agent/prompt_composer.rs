...
pub fn build_task_section(native_tools: bool) -> String {
    ...
    section.push_str(
        "You are VelaClaw. Prioritize the user's request over meta-commentary.\n\n\
         ...
         - Shell: one admit-safe invoke per tool round (simple argv or ssh <alias> <simple>). \
           A pipe between simple programs is allowed. Do not use $(), backticks, ${, redirects, \
           tee, find -exec, or wrap several checks in one ssh.\n\
