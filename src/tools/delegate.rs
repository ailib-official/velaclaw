...
mod tests {
    ...
    async fn missing_agent_param() {
        let tool = DelegateTool::new(sample_agents(), None, test_security());
        let result = tool
            .execute(json!({"prompt": "test"}), &ToolExecutionContext::default())
            .await;
    ...
    async fn unknown_agent_returns_error() {
        ...
        let result = tool
            .execute(
                json!({"agent": "nonexistent", "prompt": "test"}),
                &ToolExecutionContext::default(),
            )
            .await
            .unwrap();
    ...
    async fn depth_limit_enforced() {
        ...
        let result = tool
            .execute(
                json!({"agent": "researcher", "prompt": "test"}),
                &ToolExecutionContext::default(),
            )
            .await
            .unwrap();
    ...
    async fn agentic_mode_rejects_empty_allowed_tools() {
        ...
        let result = tool
            .execute(
                json!({"agent": "agentic", "prompt": "test"}),
                &ToolExecutionContext::default(),
            )
            .await
            .unwrap();
