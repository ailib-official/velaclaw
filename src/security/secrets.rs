...
// in `~/.velaclaw/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.velaclaw/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(velaclaw_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: velaclaw_dir.join(".secret_key"),
            enabled,
        }
    }
