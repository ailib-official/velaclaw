...
function ensure_config {
    CONFIG_DIR="$HOST_TARGET_DIR/.velaclaw"
    CONFIG_FILE="$CONFIG_DIR/config.toml"
    WORKSPACE_DIR="$CONFIG_DIR/workspace"

    if [ ! -f "$CONFIG_FILE" ]; then
        echo -e "${YELLOW}⚙️  Config file missing in target/.velaclaw. Creating default dev config from template...${NC}"
...
function print_help {
    echo -e "${YELLOW}VelaClaw Development Environment Manager${NC}"
    ...
    echo -e "  ${GREEN}agent${NC}   Enter Agent (VelaClaw CLI)"
...
case "$1" in
    up)
        ...
        echo -e "   - Config: target/.velaclaw/config.toml (Edit locally to apply changes)"
        ;;
    ...
    shell)
        ...
        docker exec -it velaclaw-sandbox /bin/bash
        ;;

    agent)
        echo -e "${GREEN}🤖 Entering Agent Container (VelaClaw)... (Type 'exit' to leave)${NC}"
        docker exec -it velaclaw-dev /bin/bash
        ;;
    ...
    clean)
        echo -e "${RED}⚠️  WARNING: This will delete 'target/.velaclaw' data and Docker volumes.${NC}"
        read -p "Are you sure? (y/N) " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            docker compose -f "$COMPOSE_FILE" down -v
            rm -rf "$HOST_TARGET_DIR/.velaclaw"
