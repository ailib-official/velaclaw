...
- `velaclaw agent --plan -m "Propose a change"` — Plan phase: mutating tools are blocked (default is Build). With `[agent].bounded_dag_live = true` and empty `bounded_dag_path`, Plan runs a tool-free planner (`fast` route when configured, else session default) then prints the linear DAG; omit `--plan` (Build) to run each work node through the existing tool loop (CLI and Web share `graph_scheduler`; successful hops skip observe; parlor ≤1). Planned `tool_direct` I is one admit-safe invoke (flags and simple `ssh` pipes allowed; `&&` / `sh -c` / `xargs` rejected). Admit rejects persist `admit_reject.json` and do not execute. Empty command stdout stays a stored record and later hops see `(no output)` for that node id. A missing tool record stops and names the node. An `llm_cognition` node receives no tools. Empty assistant text after tools is stored as an internodal hop artifact when a tool gist exists; hops and graph-end delivery fail closed if artifacts are empty or miss declared evidence layers. Live work hops do not inject `.velaclaw` scratch listings as task retrieve; allowlist misses Ask (Once this session) instead of continuing the hop as success. Live work hops with speed/tools/document capabilities use configured hints rather than pinning the session default; hop/graph evidence gates read the tool accumulator so listing plus prose is not success. Parlor reads the full graph `dag_art` bundle, not only the last hop body. All-LLM graphs that declare readonly evidence layers are rewritten to ToolDirect I at admit; two workspace-listing shell rounds without evidence-layer progress close the hop. Short Build approvals reuse the session DAG; other Build text replans. Same `host_phase` contract as Web chat.
...
`[agent].bounded_dag_live` is a **separate** opt-in (default `false`): CLI + Web only. The first planner-model hop is the path: `chat_only` includes the full reply; `{path:single_work}` or invalid / empty `chat_only` is **one native tool loop**. A 1-node non-ToolDirect DAG collapses to that loop; 2+ nodes or a 1-node ToolDirect stay the graph. There is no 1-node split-refine hop. Multi-deliverable work is a DAG JSON object (2–8 nodes; linear or `fork` ready-set). Planner keeps the session cheap default (not the picker); **work hops use the Web picker** when the request has `provider/model` via Route(C, Context) (VL-NA-043/045 / VL-APE-002). Successful hops skip observe; CLI and Web share `graph_scheduler` (VL-APE-001). LLM hops default to native tools; tool-only nodes call `execute_tool_batch` (VL-APE-003). Independent tool-only nodes may share one `execute_tool_batch` wave (VL-APE-004). Cap/PolicyDeny hops skip observe (VL-NA-045). The hop consumes **prepared session history** (VL-CTX-001 / VL-NA-030); skipping Envelope is only Plan-phase preview (VL-NA-019), not “DAG turns have no session.” Typed fail freezes the completed prefix and may replan later (VL-APE-004); success never splices remaining. Not L4 `candidate_dag_emit`. See [config-reference.md](config-reference.md#agent).
