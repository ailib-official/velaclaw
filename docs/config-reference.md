...
| `bounded_dag_live` | `false` | **VL-NA-011/015/020/025/026 (opt-in).** Cheap JSON judge picks `chat_only` (no tools), `single_work` (one tool-loop hop), or `plan_dag` (planner then linear work nodes). Invalid judge JSON is `single_work`, never silent `chat_only`. Work hops and live chat/single use the session default model (not CostRouter). After each hop, observe may `replan_remaining` once. Dist default off. Not L4 `candidate_dag_emit`. |
