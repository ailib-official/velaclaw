...
`[agent].bounded_dag_live` is a **separate** opt-in (default `false`): CLI + Web only. The first planner-model hop is the path: `chat_only` includes the full reply; any tool work is a linear DAG JSON object (1–8 nodes). `{path:single_work}` or invalid / empty `chat_only` becomes a 1-node graph, then one split-refine chat before tools — not a second tool-loop entry. Planner/observe keep the session default model; **work hops use the Web picker** when the request has `provider/model` (VL-NA-043/045). Cap/PolicyDeny hops skip observe (VL-NA-045). The hop consumes **prepared session history** (VL-CTX-001 / VL-NA-030); skipping Envelope is only Plan-phase preview (VL-NA-019), not “DAG turns have no session.” After each hop, observe can replan remaining once (15s timeout fail-open); **stop** is coerced to continue while remaining nodes > 0. Not L4 `candidate_dag_emit`. See [config-reference.md](config-reference.md#agent).
