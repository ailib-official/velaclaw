//! L2 workspace `agent-policy.yaml` loading and validation (VL-ARCH-006).
...
impl AgentPolicyLayer {
    ...
    pub fn discover_and_load(config: &crate::config::Config) -> Result<Option<Self>> {
...
/// Walk cwd (and `VELACLAW_WORKSPACE` / config workspace roots) for `agent-policy.yaml`.
pub fn discover_agent_policy_path(config: &crate::config::Config) -> Result<Option<PathBuf>> {
...
"#,
        ...
        assert_eq!(
            policy.self_adjust.as_ref().unwrap().allowed_writes,
            vec!["memory.preferences.tone".to_string()]
        );
    }
...
"#,
        ...
        assert!(
            err.to_string().contains("secret fields"),
    ...
    fn discover_from_cwd_walk_up() {
        ...
        let found = discover_agent_policy_path(&config).unwrap();
