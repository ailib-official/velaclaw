...
usage() {
  cat <<'USAGE'
VelaClaw installer bootstrap engine

Usage:
  ./velaclaw_install.sh [options]
  ./bootstrap.sh [options]         # compatibility entrypoint

Modes:
  Default mode installs/builds VelaClaw only (requires existing Rust toolchain).
...
Examples:
  ./velaclaw_install.sh
  ./velaclaw_install.sh --guided
  ./velaclaw_install.sh --install-system-deps --install-rust
  ./velaclaw_install.sh --prefer-prebuilt
  ./velaclaw_install.sh --prebuilt-only
  ./velaclaw_install.sh --onboard --api-key "sk-..." --provider openai/gpt-5.2
  ./velaclaw_install.sh --interactive-onboard
...
Environment:
  VELACLAW_CONTAINER_CLI     Container CLI command (default: docker; auto-fallback: podman)
  VELACLAW_DOCKER_DATA_DIR   Host path for Docker config/workspace persistence
  VELACLAW_DOCKER_IMAGE      Docker image tag to build/run (default: velaclaw-bootstrap:local)
  VELACLAW_API_KEY           Used when --api-key is not provided
  VELACLAW_PROVIDER          Used when --provider is not provided (default: openrouter)
  VELACLAW_MODEL             Used when --model is not provided
  VELACLAW_BOOTSTRAP_MIN_RAM_MB   Minimum RAM threshold for source build preflight (default: 2048)
  VELACLAW_BOOTSTRAP_MIN_DISK_MB  Minimum free disk threshold for source build preflight (default: 6144)
  VELACLAW_DISABLE_ALPINE_AUTO_DEPS
...
should_attempt_prebuilt_for_resources() {
  ...
  min_ram_mb="${VELACLAW_BOOTSTRAP_MIN_RAM_MB:-2048}"
  min_disk_mb="${VELACLAW_BOOTSTRAP_MIN_DISK_MB:-6144}"
...
install_prebuilt_binary() {
  ...
  archive_url="https://github.com/velaclaw-labs/velaclaw/releases/latest/download/velaclaw-${target}.tar.gz"
  temp_dir="$(mktemp -d -t velaclaw-prebuilt-XXXXXX)"
  archive_path="$temp_dir/velaclaw-${target}.tar.gz"
  ...
  extracted_bin="$temp_dir/velaclaw"
  if [[ ! -x "$extracted_bin" ]]; then
    extracted_bin="$(find "$temp_dir" -maxdepth 2 -type f -name velaclaw -perm -u+x | head -n 1 || true)"
  fi
  if [[ -z "$extracted_bin" || ! -x "$extracted_bin" ]]; then
    warn "Archive did not contain an executable velaclaw binary."
    rm -rf "$temp_dir"
    return 1
  fi
  ...
  mkdir -p "$install_dir"
  install -m 0755 "$extracted_bin" "$install_dir/velaclaw"
  rm -rf "$temp_dir"

  info "Installed pre-built binary to $install_dir/velaclaw"
...
run_pacman() {
  ...
  local pacman_rc=0
  pacman_cfg_tmp="$(mktemp /tmp/velaclaw-pacman.XXXXXX.conf)"
...
run_guided_installer() {
  ...
  echo
  echo "VelaClaw guided installer"
  ...
  if prompt_yes_no "Install velaclaw into cargo bin now?" "yes"; then
    SKIP_INSTALL=false
  else
    SKIP_INSTALL=true
  fi
...
resolve_container_cli() {
  local requested_cli
  requested_cli="${VELACLAW_CONTAINER_CLI:-docker}"
  ...
  if [[ "$requested_cli" != "docker" ]]; then
    error "Set VELACLAW_CONTAINER_CLI to an installed Docker-compatible CLI (e.g., docker or podman)."
  else
    error "Install Docker, install podman, or set VELACLAW_CONTAINER_CLI to an available Docker-compatible CLI."
  fi
  exit 1
}
...
run_docker_bootstrap() {
  ...
  docker_image="${VELACLAW_DOCKER_IMAGE:-velaclaw-bootstrap:local}"
  fallback_image="ghcr.io/velaclaw-labs/velaclaw:latest"
  if [[ "$TEMP_CLONE" == true ]]; then
    default_data_dir="$HOME/.velaclaw-docker"
  else
    default_data_dir="$WORK_DIR/.velaclaw-docker"
  fi
  docker_data_dir="${VELACLAW_DOCKER_DATA_DIR:-$default_data_dir}"
  DOCKER_DATA_DIR="$docker_data_dir"

  mkdir -p "$docker_data_dir/.velaclaw" "$docker_data_dir/workspace"
  ...
  if [[ "$SKIP_BUILD" == false ]]; then
    ...
    if ! "$CONTAINER_CLI" image inspect "$docker_image" >/dev/null 2>&1; then
      ...
      info "Pulling official VelaClaw image ($fallback_image)"
  ...
  config_mount="$docker_data_dir/.velaclaw:/velaclaw-data/.velaclaw"
  workspace_mount="$docker_data_dir/workspace:/velaclaw-data/workspace"
...
Use either:
  --api-key "sk-..."
or:
  VELACLAW_API_KEY="sk-..." ./velaclaw_install.sh --docker
or run interactive:
  ./velaclaw_install.sh --docker --interactive-onboard
MSG
      exit 1
    fi
    if [[ -n "$MODEL" ]]; then
  ...
  "$CONTAINER_CLI" run --rm -it \
    ...
    -e HOME=/velaclaw-data \
    -e VELACLAW_WORKSPACE=/velaclaw-data/workspace \
    -v "$config_mount" \
    -v "$workspace_mount" \
    "$docker_image" \
    "${onboard_cmd[@]}"
...
REPO_URL="https://github.com/velaclaw-labs/velaclaw.git"
ORIGINAL_ARG_COUNT=$#
GUIDED_MODE="auto"
...
CONTAINER_CLI="${VELACLAW_CONTAINER_CLI:-docker}"
API_KEY="${VELACLAW_API_KEY:-}"
PROVIDER="${VELACLAW_PROVIDER:-openrouter}"
MODEL="${VELACLAW_MODEL:-}"
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  if [[ "$OS_NAME" == "Linux" && -z "${VELACLAW_DISABLE_ALPINE_AUTO_DEPS:-}" ]] && have_cmd apk; then
    find_missing_alpine_prereqs
    if [[ ${#ALPINE_MISSING_PKGS[@]} -gt 0 && "$INSTALL_SYSTEM_DEPS" == false ]]; then
      ...
      info "Auto-enabling system dependency installation (set VELACLAW_DISABLE_ALPINE_AUTO_DEPS=1 to disable)."
      INSTALL_SYSTEM_DEPS=true
    fi
  fi
...
if [[ ! -f "$WORK_DIR/Cargo.toml" ]]; then
  if [[ -f "$(pwd)/Cargo.toml" ]]; then
    ...
    TEMP_DIR="$(mktemp -d -t velaclaw-bootstrap-XXXXXX)"
...
Your containerized VelaClaw data is persisted under:
DONE
...
Next steps:
  ./velaclaw_install.sh --docker --interactive-onboard
  ./velaclaw_install.sh --docker --api-key "sk-..." --provider openai/gpt-5.2
DONE
  exit 0
fi
...
or re-run with:
  ./velaclaw_install.sh --install-rust
MSG
  exit 1
fi
...
if [[ "$SKIP_INSTALL" == false ]]; then
  info "Installing velaclaw to cargo bin"
...
VELACLAW_BIN=""
if have_cmd velaclaw; then
  VELACLAW_BIN="velaclaw"
elif [[ -x "$HOME/.cargo/bin/velaclaw" ]]; then
  VELACLAW_BIN="$HOME/.cargo/bin/velaclaw"
elif [[ -x "$WORK_DIR/target/release/velaclaw" ]]; then
  VELACLAW_BIN="$WORK_DIR/target/release/velaclaw"
fi

if [[ "$RUN_ONBOARD" == true ]]; then
  if [[ -z "$VELACLAW_BIN" ]]; then
    error "onboarding requested but velaclaw binary is not available."
    error "Run without --skip-install, or ensure velaclaw is in PATH."
    exit 1
  fi

  if [[ "$INTERACTIVE_ONBOARD" == true ]]; then
    info "Running interactive onboarding"
    "$VELACLAW_BIN" onboard --interactive
  else
    if [[ -z "$API_KEY" ]]; then
      cat <<'MSG'
==> Onboarding requested, but API key not provided.
Use either:
  --api-key "sk-..."
or:
  VELACLAW_API_KEY="sk-..." ./velaclaw_install.sh --onboard
or run interactive:
  ./velaclaw_install.sh --interactive-onboard
MSG
      exit 1
    ...
    ONBOARD_CMD=("$VELACLAW_BIN" onboard --api-key "$API_KEY" --provider "$PROVIDER")
    if [[ -n "$MODEL" ]]; then
      ONBOARD_CMD+=(--model "$MODEL")
    fi
    "${ONBOARD_CMD[@]}"
