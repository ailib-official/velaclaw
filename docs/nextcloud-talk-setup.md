...
This guide covers native Nextcloud Talk integration for VelaClaw.
...
Add this section in `~/.velaclaw/config.toml`:
...
velaclaw daemon
# or
velaclaw gateway --host 127.0.0.1 --port 3000
```
...
When `webhook_secret` is configured, VelaClaw verifies:
...
- VelaClaw ignores bot-originated webhook events (`actorType = bots`).
- VelaClaw ignores non-message/system events.
...
3. Confirm VelaClaw receives and replies in the same room.
