...
Tài liệu này là nguồn tham khảo chính thức về cấu hình channel trong VelaClaw.
...
6. **Trạng thái runtime cũ**: config đã thay đổi nhưng `velaclaw daemon` chưa được khởi động lại.

---

## 1. Namespace cấu hình

Tất cả cài đặt channel nằm trong `channels_config` trong `~/.velaclaw/config.toml`.
...
Khi chạy `velaclaw channel start` (hoặc chế độ daemon), Telegram và Discord hỗ trợ chuyển đổi runtime theo phạm vi người gửi:
...
- Xem trước bộ nhớ cache model từ `velaclaw models refresh --provider <ID>`.
- Đây là lệnh chat runtime, không phải lệnh con CLI.

## Giao thức marker hình ảnh đầu vào

VelaClaw hỗ trợ đầu vào multimodal qua các marker nội tuyến trong tin nhắn:
...
Nếu `[channels_config.matrix]` có mặt nhưng binary được build mà không có `channel-matrix`, các lệnh `velaclaw channel list`, `velaclaw channel doctor`, và `velaclaw channel start` sẽ ghi log rằng Matrix bị bỏ qua có chủ ý trong bản build này.
...
access_token = "syt_..."
user_id = "@velaclaw:matrix.example.com"   # tùy chọn, khuyến nghị cho E2EE
...
port = 6697
nickname = "velaclaw-bot"
username = "velaclaw"              # tùy chọn
channels = ["#velaclaw"]
allowed_users = ["*"]
...
5. Khởi động lại `velaclaw daemon` sau khi thay đổi config.
...
```bash
RUST_LOG=info velaclaw daemon 2>&1 | tee /tmp/velaclaw.log
```

Sau đó lọc các sự kiện channel/gateway:

```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Webhook|Channel" /tmp/velaclaw.log
```
