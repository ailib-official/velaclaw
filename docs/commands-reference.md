...
- `velaclaw agent --plan -m "Propose a change"` — Plan phase: mutating tools are blocked (default is Build). `[agent].bounded_dag_live` does not change this: the key is retained and does not run a planner or linear nodes (VL-RAO-001).
...
`[agent].bounded_dag_live` is retained (default `false`) and does not schedule a planner or linear work nodes (VL-RAO-001). CLI, Web, and channel turns use `prepare_turn_history` then `run_tool_call_loop`. A `true` value is logged and left as written in the config file. `--plan` blocks mutating tools. It does not print a DAG preview. Not L4 `candidate_dag_emit`. See [config-reference.md](config-reference.md#agent).
