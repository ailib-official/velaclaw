# Hardware Peripherals Design — VelaClaw

VelaClaw enables microcontrollers (MCUs) and Single Board Computers (SBCs) to **dynamically interpret natural language commands**, generate hardware-specific code, and execute peripheral interactions in real-time.

## 1. Vision

**Goal:** VelaClaw acts as a hardware-aware AI agent that:
...
**Mental model:** VelaClaw = brain that understands hardware. Peripherals = arms and legs it controls.
...
VelaClaw runs **directly on the device**. The board spins up a gRPC/nanoRPC server and communicates with peripherals locally.
...
2. VelaClaw fetches board-specific docs (e.g. ESP32 GPIO mapping)
...
VelaClaw runs on the **host** and maintains a hardware-aware link to the target. Used for development, introspection, and flashing.
...
2. VelaClaw identifies connected hardware (VID/PID, architecture)
...
2. VelaClaw writes/flashes via OpenOCD or probe-rs
3. Confirms success

**Or:**
1. VelaClaw auto-discovers: *"STM32 Nucleo on /dev/ttyACM0, ARM Cortex-M4"*
...
|------------------|--------------------------------|----------------------------------|
| VelaClaw runs on | Device (ESP32, RPi)           | Host (Mac, Linux)                |
...
Host runs VelaClaw; peripheral runs minimal firmware. Simple JSON over serial.

### Mode B: RPi as Host (Native GPIO)

VelaClaw on Pi; GPIO via rppal or sysfs. No separate firmware.
...
1. **Startup:** VelaClaw loads config, sees `peripherals.boards`.
...
For low-latency, typed RPC between VelaClaw and peripherals:
...
- **velaclaw-firmware** or **velaclaw-peripheral** — a separate crate/workspace.
...
- User flashes this to the board; VelaClaw connects and discovers capabilities.

## 9. Implementation Phases

### Phase 1: Skeleton ✅ (Done)

- [x] Add `Peripheral` trait, config schema, CLI (`velaclaw peripheral list/add`)
- [x] Add `--peripheral` flag to agent
- [x] Document in AGENTS.md

### Phase 2: Host-Mediated — Hardware Discovery ✅ (Done)

- [x] `velaclaw hardware discover`: enumerate USB devices (VID/PID)
- [x] Board registry: map VID/PID → architecture, name (e.g. Nucleo-F401RE)
- [x] `velaclaw hardware introspect <path>`: memory map, peripheral list
...
- [x] VelaClaw on Raspberry Pi (native GPIO via rppal)
...
- [x] `velaclaw-esp32` firmware crate (`firmware/velaclaw-esp32`) — GPIO over UART
- [x] ESP32 in hardware registry (CH340 VID/PID)
- [ ] VelaClaw *on* ESP32 (WiFi + LLM, edge-native) — future
- [ ] Wasm or template-based execution for LLM-generated logic

**Usage:** Flash `firmware/velaclaw-esp32` to ESP32, add `board = "esp32"`, `transport = "serial"`, `path = "/dev/ttyUSB0"` to config.
...
- Running full VelaClaw *on* bare STM32 (no WiFi, limited RAM) — use Host-Mediated instead
...
> *"Boards like ESP, Raspberry Pi, or boards with WiFi can connect to an LLM (Gemini or open-source). VelaClaw runs on the device, creates its own gRPC, spins it up, and communicates with peripherals. User asks via WhatsApp: 'move X arm' or 'turn on LED'. VelaClaw gets accurate documentation, writes code, executes it, stores it optimally, runs it, and turns on the LED — all on the development board.*
>
> *For STM Nucleo connected via USB/J-Link/Aardvark to my Mac: VelaClaw from my Mac accesses the hardware, installs or writes what it wants on the device, and returns the result. Example: 'Hey VelaClaw, what are the available/readable addresses on this USB device?' It can figure out what's connected where and suggest."*
