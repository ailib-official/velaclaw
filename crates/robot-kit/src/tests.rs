...
mod unit_tests {
    ...
    async fn drive_forward_mock() {
        ...
        let result = tool
            .execute(
                json!({"action": "forward", "distance": 1.0}),
    ...
    async fn drive_stop_always_succeeds() {
        ...
        let result = tool
            .execute(json!({"action": "stop"}), &ToolExecutionContext::default())
            .await
            .unwrap();
    ...
    async fn drive_rotate() {
        ...
        let result = tool
            .execute(
                json!({"action": "rotate_left", "distance": 90.0}),
                &ToolExecutionContext::default(),
            )
            .await
            .unwrap();
    ...
    async fn drive_invalid_action_fails() {
        ...
        let result = tool
            .execute(json!({"action": "fly"}), &ToolExecutionContext::default())
            .await
            .unwrap();
    ...
    async fn emote_happy() {
        ...
        let result = tool
            .execute(
                json!({"expression": "happy", "duration": 0}),
                &ToolExecutionContext::default(),
            )
            .await
            .unwrap();
    ...
    async fn emote_all_expressions_valid() {
        ...
        for expr in expressions {
            let result = tool
                .execute(
                    json!({"expression": expr, "duration": 0}),
                    &ToolExecutionContext::default(),
                )
                .await
                .unwrap();
    ...
    async fn drive_then_sense_workflow() {
        ...
        if scan.output.contains("CLEAR") {
            let drive_result = drive
                .execute(
                    json!({"action": "forward", "distance": 0.5}),
                    &ToolExecutionContext::default(),
                )
                .await
                .unwrap();
