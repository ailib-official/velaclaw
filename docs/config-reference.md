...
| `host_decide` | `false` | **ORCH-HOST-001 (opt-in):** when true, select turn model via CAP reachable ∩ host Decide ranking (BYOK embed; not Prism Gateway HTTP). Default **off** — explicit configured model path unchanged. Optimize via `host_decide_optimize` (`cost` \| `latency` \| `balanced`). Observe: `velaclaw doctor host-decide --force`. Requires `--features ai-protocol`. |
| `host_decide_optimize` | `cost` | Optimize goal used when `host_decide` is enabled. |
| `candidate_dag_emit` | `false` | **ORCH-DAG-EMIT-001 (opt-in):** when true, host helpers may call schema-strict candidate emit → L2 fallback (`orchestration::dag_emit::maybe_emit_candidate`). Default **off**. Does **not** turn on AI-DAG in the live agent chat loop. Observe: `velaclaw doctor dag-emit --candidate <path>`. Requires `--features ai-protocol`. |
